...
- **Google Kubernetes Engine cluster locations** — add a cluster location to a team's Kubernetes configuration; two modes are supported:
  ...
  - **OPA Gatekeeper constraints** — add team-supplied `ConstraintTemplate` + `Constraint` pairs under `gatekeeper_constraints` in the team's `google_kubernetes_engine_clusters` block; keys must be unique across all teams
- **Feature flags** — enable or disable optional features:
  ...
  - **Kubernetes-level flags** (set on `google_kubernetes_engine_clusters`, not a repository):
    ...
    - `gatekeeper_constraints` — map of team-supplied OPA Gatekeeper ConstraintTemplate and Constraint pairs to deploy alongside platform-managed policies (default: `{}`); keys must be unique across all teams to avoid merge collisions
